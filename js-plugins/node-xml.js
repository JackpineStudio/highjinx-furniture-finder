...
(function() {
	...
	XMLP.prototype.next = function() {
		if (!this.m_pause) {
			...
			// save off the current event loop state and set the state to
			// interrupt
	...
	XMLP.prototype._parseNamespacesAndAtts = function(atts) {
		// translate namespaces into objects with "prefix","uri", "scopetag" Add
		// them to: this.m_namespaceList
		// The function should return a new list of tag attributes with the
		// namespaces filtered
		that = this;
		var newnamespaces = [];
		var filteredatts = [];
	...
	XMLP.prototype._getContextualNamespace = function(prefix) {
		...
		// no match was found for the prefix so pop off the first non-prefix
		// namespace
	...
	XMLP.prototype._removeExpiredNamesapces = function(closingtagname) {
		// remove the expiring namespaces from the list (you can id them by
		// scopetag)
		var keeps = [];
	...
	XMLP.prototype._parseCDATA = function(iB) {
		...
		if (iE == -1) {
			// This item never closes, although it could be a malformed
			// document, we will assume that we are mid-chunck, save the string
			// and reurn as interrupted
			this.m_chunkTransitionContinuation = this.m_xml.slice(iB - 9);// the
			// '-<![CDATA[
			// adds
			// the
			// '<!DOCTYPE'
	...
	XMLP.prototype._parseComment = function(iB) {
		...
		if (iE == -1) {
			...
			// document, we will assume that we are mid-chunck, save the string
			// and reurn as interrupted
			this.m_chunkTransitionContinuation = this.m_xml.slice(iB - 4);// the
			// '-4'
			// adds
			// the
			// '<!--'
	...
	XMLP.prototype._parseElement = function(iB) {
		...
		if (iE == -1) {
			// This element never closes, although it could be a malformed
			// document, we will assume that we are mid-chunck, save the string
			// and reurn as interrupted
			this.m_chunkTransitionContinuation = this.m_xml.slice(iB - 1);// the
			// '-1'
			// adds
			// the
			// '<'
		...
		/*
		 * if(SAXStrings.indexOfNonWhitespace(this.m_xml, iB, iDE) != iB) return
		 * this._setErr(XMLP.ERR_ELM_NAME);
		 */
		this._clearAttributes();
	...
	XMLP.prototype._parsePI = function(iB) {
		...
		if (iE == -1) {
			...
			// document, we will assume that we are mid-chunck, save the string
			// and reurn as interrupted
			this.m_chunkTransitionContinuation = this.m_xml.slice(iB - 2);// the
			// '-2'
			// adds
			// the
			// '?>'
	...
	// SaxParser is an object that basically wraps an XMLP instance, and
	// provides an
	// event-based interface for parsing. This is the object users interact with
	// when coding
	// with XML for <SCRIPT>
	...
	SaxParser.prototype.parseFile = function(filename) { // This function
		// will only work in
		// the node.js
		// environment.
		var fs = require('fs');
	...
	SaxParser.prototype._fireEvent = function(iEvt) {
		var hnd = 0, func = 0, args = arguments, iLen = args.length - 1;
	...
	SaxParser.prototype._parseLoop = function(parser) {
		...
		while (!this.m_bErr) {
			...
			if (iEvent == XMLP._ELM_B) {
				...
				this._fireEvent(SaxParser.ELM_E, nameobject.name,
				...
				this._fireEvent(SaxParser.ELM_E, nameobject.name,
						(nameobject.prefix === '') ? null : nameobject.prefix,
						(theuri === '') ? null : theuri);
				// this._fireEvent(SaxParser.ELM_B, parser.getName(),
				// this.m_parser.m_atts.map(function(item){return { name :
				// item[0], value : item[1], };}) );
				// this._fireEvent(SaxParser.ELM_E, parser.getName());
			} else if (iEvent == XMLP._TEXT) {
				this._fireEvent(SaxParser.CHARS, parser.getContent().slice(
						parser.getContentBegin(), parser.getContentEnd()));
			} else if (iEvent == XMLP._ENTITY) {
				this._fireEvent(SaxParser.CHARS, parser.getContent(), parser
						.getContentBegin(), parser.getContentEnd() - parser.getContentBegin());
			} else if (iEvent == XMLP._PI) {
				this._fireEvent(SaxParser.PI, parser.getName(), parser
						.getContent().substring(parser.getContentBegin(),
								parser.getContentEnd()));
			} else if (iEvent == XMLP._CDATA) {
				this._fireEvent(SaxParser.CD_B, parser.getContent().slice(
						parser.getContentBegin(), parser.getContentEnd()));
				// this._fireEvent(SaxParser.CHARS, parser.getContent(),
				// parser.getContentBegin(), parser.getContentEnd() -
				// parser.getContentBegin());
				// this._fireEvent(SaxParser.CD_E);
			} else if (iEvent == XMLP._COMMENT) {
				this._fireEvent(SaxParser.CMNT, parser.getContent().slice(
						parser.getContentBegin(), parser.getContentEnd()));
