	...
	rss.parseURL(feed, function(articles) {	
		for(var i = 0; i < (articles.length); i++){
			...
			if(article.description.indexOf("<table") != -1){
				...
				article.image = "none";
				if(article.link.indexOf("kijiji") != -1) {
					article.image = "logos/kijiji.png";
				}else if(article.link.indexOf("craigslist") != -1) {
					article.image = "logos/craigslist.png";
				} else {
					article.image = "logos/usedOttawa.png";
				}
			}
			...
			var parser = new htmlparser.Parser({
				...
				onclosetag :function(tagname) {
					if(tagname == "tr" && tagOpen) {
						tagOpen = false;
						item = 0;
						if(objectDetails['image'] == " " || objectDetails['image'] == 'none' || objectDetails['image'] == "notSet") {
							objectDetails['image'] = "logos/kijiji.png";
						}
						var object = new SaleObject(objectDetails['title'], objectDetails['link'], objectDetails['description'], objectDetails['image']);
						object.setPageNumber(pageNum + (row / 100));
						databaseHandler.insertSingleItemIntoDatabase(object);
						row++;
