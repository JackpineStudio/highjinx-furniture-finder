	...
	if (object != null) {
		...
		.on('end', function() {
			if (count == 0) {
				...
				var sqlQuery = "UPDATE SaleObjects SET pageNumber = ? where link = ?";
				var query = connection.query(sqlQuery, [object.getPageNumber(), object.getLink()]);
				query.on('end', function() {
...
function getObjectsFromDatabase(objects, callback, callback2, connection) {
	var sqlQuery = 'SELECT * FROM SaleObjects ORDER BY pageNumber, dateAdded DESC';	
	var query = connection.query(sqlQuery);
	Logger.log(0, "Executing query '" + query.sql + "'");
	query
	.on('result', function(row){
