/*******************************************************************************
 * node-rss - an RSS parser for node. http://github.com/ibrow/node-rss
 * 
 * Copyright (c) 2010 Rob Searles http://www.robsearles.com
 * 
 * node-rss is released under the MIT license - see LICENSE for more info
 * 
 * ******************************************************************** node-rss
 * makes heavy use of the node-xml module written by Rob Righter -
 * 
 * @robrighter http://github.com/robrighter/node-xml
 ******************************************************************************/
...
exports.parseURL = function(url, cb) {
	...
	function get_rss(URL) {
		...
		var request; // = client.request('GET', parts.pathname, {'host':
						// parts.hostname});
		...
		request.addListener('response', function(response) {
			...
			default:
				/*
				 * response.setEncoding('utf8'); response.addListener('data',
				 * function (chunk) { //sys.puts('BODY: ' + chunk); });
				 */
				break;
			}
		});
